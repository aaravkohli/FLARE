...
And hosts:
  ...
  - h2 (Drone Client) connected to s1
"""
...
def setup_topology():
    ...
    info('*** Creating links\n')
    # Ingress Drone host connection
    # h2 connects to s1 Port 4
    ...
    # Run iperf UDP traffic generator in background on h2 (Drone client) sending 1Mbps stream
    info('*** Running iperf client UDP stream on h2\n')
    h2.cmd('iperf -c 10.0.0.1 -u -b 1M -t 999999 -i 1 > /app/logs/iperf_client.log 2>&1 &')

    # Start a background ping monitor to log latency and packet loss
    info('*** Starting ping connectivity monitor\n')
    h2.cmd('ping -i 0.5 10.0.0.1 > /app/logs/ping_monitor.log 2>&1 &')
